...
let piShort = (Number(pi.toFixed(2))); //toFixed() возвращает число с () после строки
...
let min = Math.min.apply(null, arr); //Math min and max не работают с this для этого null,метод apply принимает массив не как список (call)
...
let randomShort = ((Math.random().toFixed(2))); //Math.random() случайное число от 0 до 1 
...
let b = parseInt("‘100$’".replace(/\D+/g, "")); // parseInt строка в число, /\D+/g регулярное выражение ( /\D/ ищет не цифровой символ,/g глобально )
console.log(b); //100